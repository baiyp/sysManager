var TableDatatablesManaged = function () {
	...
	var initTable3 = function () {
        ...
        table.dataTable({
            ...
            "processing": true,
            "serverSide": true,
            "ajax": WebUtil.getMainRoot()+"/queryAdvertisement",
            "displayStart": 0,
            "pageLength": 15,
            "lengthMenu": [
                [6, 15, 20, -1],
	...
	var batchDelete = function(){
		...
		$("#batch-delete").on("click",function(){
			 ...
			 if(batch.length > 0){
				 bootbox.setLocale("zh_CN");
	            	bootbox.confirm("你确定要删除所选的轮播图片吗?", function(result) {
	                   	if(result == true){
	                   		WebUtil.ajaxRequest(WebUtil.getMainRoot()+'/deleteAdvertisement',{"advertId":batch.join()},loadDataTables);
	                   	}
	                });
